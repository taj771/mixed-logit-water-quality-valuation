...
# Large-text version for presentations
#######################################################################################
...
#load model
model <- apollo_loadModel("analysis/outputs/models/Model 2")
...
p <- ggplot(df_all, aes(x = `WQ change scenario`, y = Estimate, color = `WQ change scenario`)) +
  ...
  geom_point(position = position_dodge(width = 0.1), size = 6) +
  geom_errorbar(
    aes(ymin = Estimate - SE, ymax = Estimate + SE),
    position = position_dodge(width = 0.1), width = 0.2
  ) +
  # Axis formatting
  ...
  theme(
    ...
    axis.text.x = element_text(hjust = 1, size = 44, angle = 0),
...
# Save to PNG
ggsave("analysis/outputs/figures/marginal_WTP_largetext.png", plot = p, width = 20, height = 16, units = "in", dpi = 300)
