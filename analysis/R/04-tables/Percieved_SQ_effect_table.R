...
estimates_df <- read.csv("analysis/outputs/models/Model 8_estimates.csv")%>%
  ...
  # A normal mixing SD is identified only up to sign; Model 5's counterpart comes
  # out NEGATIVE from the same data, so a re-estimation could put a negative
  # number in a column headed "Standard Deviation". Matches Table Codes.R.
  mutate(Estimate = ifelse(str_starts(X, "sigma"), abs(Estimate), Estimate))%>%
  # round(x, 3) collapsed b_cost (-0.00495264) to -0.005.
...
df3 <- estimates_df %>%
  filter(
    str_starts(Parameter, "b_cost") |
      ...
      str_starts(Parameter, "b_basesq_worse") |
      str_starts(Parameter, "b_basesq_better")
  ) %>%
...
model <- apollo_loadModel("analysis/outputs/models/Model 8")  # Replace "mymodel" with your actual modelName
...
Model4.6 <- rbind(df_model4, new_row) %>%
  # was `Model 4.6` - this table is Model 8 (N 3,671 / 25,803)
  rename(`Model 8` = Mean) %>%
  mutate(
    Parameter = factor(Parameter, levels = c(
      ...
      "b_basesq_worse", "se_b_basesq_worse",
      "b_basesq_better", "se_b_basesq_better",
  ...
  arrange(Parameter)%>%
  rename(Standard_Deviation_m8 = Standard_Deviation)
...
Table2 <- Model4.6%>%
  ...
  mutate(Parameter = if_else(Parameter == "b_basesq_worse", "Percieved WQ:Worse", Parameter))%>%
  mutate(Parameter = if_else(Parameter== "b_basesq_better", "Percieved WQ:Better", Parameter))%>%
...
# Save to .tex file
print(latex_table, file = "analysis/outputs/tables/Table_appendix_perception_SQ.tex", include.rownames = FALSE)
